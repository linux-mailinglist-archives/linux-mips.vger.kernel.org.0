...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id BDE8E42D544
	for <lists+linux-mips@lfdr.de>; Thu, 14 Oct 2021 10:38:57 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S230094AbhJNIlA (ORCPT <rfc822;lists+linux-mips@lfdr.de>);
        Thu, 14 Oct 2021 04:41:00 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:44198 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S230042AbhJNIlA (ORCPT
        <rfc822;linux-mips@vger.kernel.org>); Thu, 14 Oct 2021 04:41:00 -0400
Received: from mail-qt1-x834.google.com (mail-qt1-x834.google.com [IPv6:2607:f8b0:4864:20::834])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 76E86C061570;
        Thu, 14 Oct 2021 01:38:55 -0700 (PDT)
Received: by mail-qt1-x834.google.com with SMTP id y11so5040457qtn.13;
        Thu, 14 Oct 2021 01:38:55 -0700 (PDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=gmail.com; s=20210112;
        h=mime-version:references:in-reply-to:from:date:message-id:subject:to
         :cc:content-transfer-encoding;
        bh=GV27/t/Nzpux8qOv1/v5YlkLzP0RuCr1k+LYdE+HTEY=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        bh=GV27/t/Nzpux8qOv1/v5YlkLzP0RuCr1k+LYdE+HTEY=;
        b=
X-Gm-Message-State: AOAM532orx47TCYA1cQGqx4ulStrUrj9nd1kgZY/ZhyRxRYn7ukiyhQ3
        vxkUkpFaIYUgSrTGe0IRNTuaPK/O3tHbjy1vD5ocf2t7aL4=
X-Google-Smtp-Source: ABdhPJxZ3gHal5oq+6aA151MizuD17RLhmauABpJC+8VUmXdF2U40N1zYj+gXHxkwLlevmhfppj10nBvqGX302dZPXc=
X-Received: by 2002:ac8:15a:: with SMTP id f26mr4991107qtg.248.1634200734151;
 Thu, 14 Oct 2021 01:38:54 -0700 (PDT)
MIME-Version: 1.0
...
Date:   Thu, 14 Oct 2021 16:38:42 +0800
Message-ID: <CAKcpw6VDdXmsEXEYyrM=JnOxJB_F739qn=4=Fs8w2ToZ+-yb-g@mail.gmail.com>
...
Feiyang Chen <chris.chenfeiyang@gmail.com> =E4=BA=8E2021=E5=B9=B410=E6=9C=
=8814=E6=97=A5=E5=91=A8=E5=9B=9B =E4=B8=8B=E5=8D=884:32=E5=86=99=E9=81=93=
=EF=BC=9A
>
...
> diff --git a/arch/mips/include/asm/entry-common.h b/arch/mips/include/asm=
/entry-common.h
...
>  {
>         return regs->regs[29];
> @@ -156,9 +161,6 @@ static inline long regs_return_value(struct pt_regs *=
regs)
>  #define instruction_pointer(regs) ((regs)->cp0_epc)
>  #define profile_pc(regs) instruction_pointer(regs)
>
> -extern asmlinkage long syscall_trace_enter(struct pt_regs *regs, long sy=
scall);
...
> @@ -157,4 +157,9 @@ static inline int syscall_get_arch(struct task_struct=
 *task)
>         return arch;
>  }
>
...
> diff --git a/arch/mips/include/asm/thread_info.h b/arch/mips/include/asm/=
thread_info.h
...
>
>  /*
> @@ -69,6 +70,8 @@ static inline struct thread_info *current_thread_info(v=
oid)
...
> @@ -149,22 +152,10 @@ static inline struct thread_info *current_thread_in=
fo(void)
...
> -                                _TIF_SYSCALL_AUDIT | _TIF_SYSCALL_TRACEP=
OINT)
> -
...
> diff --git a/arch/mips/include/uapi/asm/ptrace.h b/arch/mips/include/uapi=
/asm/ptrace.h
...
>
> -obj-y          +=3D branch.o cmpxchg.o elf.o entry.o genex.o idle.o irq.=
o \
> -                  process.o prom.o ptrace.o reset.o setup.o signal.o \
> -                  syscall.o time.o topology.o traps.o unaligned.o watch.=
o \
> -                  vdso.o cacheinfo.o
> +obj-y          +=3D branch.o cacheinfo.o cmpxchg.o elf.o entry.o genex.o=
 \
...
>         jal     schedule_tail           # a0 =3D struct task_struct *prev
>         move    a0, s1
>         jal     s0
> -       j       syscall_exit
> +       move    a0, sp
...
>         RESTORE_SP_AND_RET
>         .set    at
> @@ -143,32 +126,6 @@ work_notifysig:                            # deal wi=
th pending signals and
...
> -       SAVE_STATIC
> -syscall_exit_work:
> -       LONG_L  t0, PT_STATUS(sp)               # returning to kernel mod=
e?
> -       andi    t0, t0, KU_USER
...
>   *
> @@ -1305,78 +1302,3 @@ long arch_ptrace(struct task_struct *child, long r=
equest,
>   out:
>         return ret;
>  }
...
> -
> -       /*
> -        * Negative syscall numbers are mistaken for rejected syscalls, b=
ut
> -        * won't have had the return value set appropriately, so we do so=
 now.
> -        */
> -       if (syscall < 0)
...
> +
> +       move    a0, a1                          # shift argument register=
s
> +       move    a1, a2
> +       move    a2, a3
> +       lw      a3, 16(sp)
...
> +        * intercept sys_sched_xxxaffinity() calls until we get a proper =
hook
> +        * in kernel/sched/core.c.  Considered only temporary we only sup=
port
> +        * these hooks for the 32-bit kernel - there is no MIPS64 MT proc=
essor
> +        * atm.
> +        */
...
> +
> +#define __SYSCALL_WITH_COMPAT(nr, native, compat)      __SYSCALL(nr, nat=
ive)
...
> +       END(sys32_syscall)
> +
> +#define __SYSCALL_WITH_COMPAT(nr, native, compat)      __SYSCALL(nr, com=
pat)
> +#define __SYSCALL(nr, entry)   PTR entry
> +       .align  3
...
> -
> -       /*
> -        * We intentionally keep the kernel stack a little below the top =
of
> -        * userspace so we don't have to do a slower byte accurate check =
here.
> -        */
> -       addu    t4, t0, 32
...
> -EXPORT(sys_call_table)
> -#include <asm/syscall_table_o32.h>
> diff --git a/arch/mips/kernel/scall64-n32.S b/arch/mips/kernel/scall64-n3=
2.S
> deleted file mode 100644
...
> -EXPORT(sysn32_call_table)
> -#include <asm/syscall_table_n32.h>
> diff --git a/arch/mips/kernel/scall64-n64.S b/arch/mips/kernel/scall64-n6=
4.S
> deleted file mode 100644
...
> - * to ABI64 calling convention.         64-bit syscalls are also process=
ed
> - * here for now.
> - */
> -#include <linux/errno.h>
...
> -       move    a0, sp
> -       /*
> -        * absolute syscall number is in v0 unless we called syscall(__NR=
_###)
> -        * where the real syscall number is in a0
...
> index c9b2a75563e1..314a6ffa0e07 100644
...
>  #include <asm/war.h>
> @@ -627,7 +626,7 @@ SYSCALL_DEFINE3(sigaction, int, sig, const struct sig=
action __user *, act,
>  #endif
>
...
>         struct pt_regs *regs;
> @@ -649,22 +648,17 @@ asmlinkage void sys_sigreturn(void)
>         else if (sig)
>                 force_sig(sig);
>
> -       /*
...
> -               : "r" (regs));
> -       /* Unreached */
> +       regs->regs[0] =3D 0;      /* No syscall restarting */
...
>         struct pt_regs *regs;
> @@ -686,21 +680,16 @@ asmlinkage void sys_rt_sigreturn(void)
>         else if (sig)
>                 force_sig(sig);
>
> +       regs->regs[0] =3D 0;      /* No syscall restarting */
...
>
>  #ifdef CONFIG_TRAD_SIGNALS
> @@ -852,11 +841,11 @@ static void handle_signal(struct ksignal *ksig, str=
uct pt_regs *regs)
...
>                 return;
> @@ -904,7 +893,7 @@ asmlinkage void do_notify_resume(struct pt_regs *regs=
, void *unused,
>
>         /* deal with pending signal delivery */
>         if (thread_info_flags & (_TIF_SIGPENDING | _TIF_NOTIFY_SIGNAL))
> -               do_signal(regs);
> +               arch_do_signal_or_restart(regs, thread_info_flags & _TIF_=
SIGPENDING);
>
>         if (thread_info_flags & _TIF_NOTIFY_RESUME)
>                 tracehook_notify_resume(regs);
> diff --git a/arch/mips/kernel/signal_n32.c b/arch/mips/kernel/signal_n32.=
c
> index 7bd00fad61af..e282f14747d2 100644
...
>         struct pt_regs *regs;
> @@ -73,21 +72,16 @@ asmlinkage void sysn32_rt_sigreturn(void)
>         else if (sig)
>                 force_sig(sig);
>
> +       regs->regs[0] =3D 0;      /* No syscall restarting */
...
> diff --git a/arch/mips/kernel/signal_o32.c b/arch/mips/kernel/signal_o32.=
c
> index 299a7a28ca33..ea6d12dd15cc 100644
...
> @@ -151,7 +150,7 @@ static int setup_frame_32(void *sig_return, struct ks=
ignal *ksig,
>         return 0;
>  }
>
...
>         struct pt_regs *regs;
> @@ -173,21 +172,16 @@ asmlinkage void sys32_rt_sigreturn(void)
>         else if (sig)
>                 force_sig(sig);
>
> +       regs->regs[0] =3D 0;      /* No syscall restarting */
...
> @@ -253,7 +247,7 @@ struct mips_abi mips_abi_32 =3D {
>  };
>
>
...
>         struct pt_regs *regs;
> @@ -275,16 +269,11 @@ asmlinkage void sys32_sigreturn(void)
>         else if (sig)
>                 force_sig(sig);
>
> -       /*
...
>
> @@ -79,10 +80,6 @@ SYSCALL_DEFINE6(mips_mmap2, unsigned long, addr, unsig=
ned long, len,
>                                pgoff >> (PAGE_SHIFT - 12));
>  }
>
...
> @@ -182,28 +179,11 @@ static inline int mips_atomic_set(unsigned long add=
r, unsigned long new)
>                 return err;
>
...
>
> -/*
> - * mips_atomic_set() normally returns directly via syscall_exit potentia=
lly
...
> +#ifdef CONFIG_MIPS32_O32
> +       /*
> +        * Hairy, the userspace application uses a different argument pas=
sing
> +        * convention than the kernel, so we have to translate things fro=
m o32
> +        * to ABI64 calling convention.
> +        *
> +        * We don't want to stumble over broken sign extensions from user=
land.
...
> +       regs->regs[26] =3D regs->regs[7]; /* save a3 for syscall restarti=
ng */
> +       regs->regs[27] =3D 0;             /* do not return directly */
> +
> +#ifdef CONFIG_32BIT
> +       if (nr >=3D __NR_O32_Linux && nr < __NR_O32_Linux + __NR_O32_Linu=
x_syscalls) {
> +               if (get_args(regs) < 0) {
> +                       ret =3D EFAULT;
> +                       goto error;
> +               }
> +               syscall_fn =3D (sys_call_fn)sys_call_table[nr - __NR_O32_=
Linux];
> +       }
> +#endif
> +
...
> +                       goto error;
> +               }
> +               syscall_fn =3D (sys_call_fn)sys32_call_table[nr - __NR_O3=
2_Linux];
> +       }
> +#endif
> +
> +#ifdef CONFIG_MIPS32_N32
> +       if (nr >=3D __NR_N32_Linux && nr < __NR_N32_Linux + __NR_N32_Linu=
x_syscalls)
> +               syscall_fn =3D (sys_call_fn)sysn32_call_table[nr - __NR_N=
32_Linux];
> +#endif
> +
> +#ifdef CONFIG_64BIT
> +       if (nr >=3D __NR_64_Linux && nr < __NR_64_Linux + __NR_64_Linux_s=
yscalls)
> +               syscall_fn =3D (sys_call_fn)sys_call_table[nr - __NR_64_L=
inux];
> +#endif
> +
...
> @@ -207,7 +207,7 @@
>  196    n32     sched_getaffinity               compat_sys_sched_getaffin=
ity
